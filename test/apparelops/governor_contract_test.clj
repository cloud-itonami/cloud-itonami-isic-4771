(ns apparelops.governor-contract-test
  ...
  (testing ":flag-quality-concern escalates for human approval, regardless of phase/confidence"
    (let [db (store/seed-db)
          ...
          result (exec-request actor "t2"
                               {:op :flag-quality-concern :store-id "store-1"
                                :patch {:concern "suspected-counterfeit jacket #4771" :confidence 0.99}}
                               ctx)]
      (is (some? result))
    ...
    (let [db (store/seed-db)
          ...
          result (exec-request actor "t2b"
                               {:op :coordinate-supply-order :store-id "store-1"
                                :patch {:item "leather outerwear collection" :quantity 1 :estimated-cost 2800.0
                                        :vendor-id "vendor-1"}}
                               ctx)]
      (is (some? result))
      (is (= 0 (count (store/coordination-log db)))
