(ns apparelops.sim
  ...
  the permanently-excluded quality-dispute-resolution/counterfeit-
...
(defn- approve! [actor tid]
  (g/run* actor {:approval {:status :approved :by "apparel-quality-coordinator-1"}} {:thread-id tid :resume? true}))

(defn -main [& _]
  (let [db (store/seed-db)
        coordinator-phase-1 {:actor-id "coord-1" :actor-role :apparel-store-coordinator :phase 1}
        coordinator-phase-3 {:actor-id "coord-1" :actor-role :apparel-store-coordinator :phase 3}
        actor (op/build db)]
    ...
    (let [r (exec-op actor "t1" {:op :log-sales-record :store-id "store-1"
                                  :patch {:units-sold 27 :returns 2 :alterations-requested 4 :stock-count-delta -29}} coordinator-phase-1)]
      (println r)
      (println "-- human apparel store coordinator approves --")
    ...
    (println (exec-op actor "t2" {:op :log-sales-record :store-id "store-1"
                                  :patch {:units-sold 19 :returns 1 :alterations-requested 2 :stock-count-delta -20}} coordinator-phase-3))
    ...
    (println (exec-op actor "t3" {:op :schedule-staffing-operation :store-id "store-1"
                                  :patch {:shift "weekend-fitting-room" :date "2026-07-20" :window "10:00-18:00"}} coordinator-phase-3))
    ...
    (let [r (exec-op actor "t5" {:op :coordinate-supply-order :store-id "store-1"
                                 :patch {:item "leather outerwear collection" :quantity 40 :estimated-cost 3200.0
                                         :vendor-id "vendor-1"}} coordinator-phase-3)]
      (println r)
      (println "-- human apparel store coordinator reviews & approves --")
    ...
    (let [r (exec-op actor "t6" {:op :flag-quality-concern :store-id "store-1"
                                 :patch {:concern "jacket #4771 has a torn lining and the leather-content label reads full-grain but the item may be a suspected-counterfeit of the branded original" :confidence 0.9}} coordinator-phase-3)]
      (println r)
      (println "-- human apparel store coordinator reviews & approves --")
    ...
    (println (exec-op actor "t9" {:op :coordinate-supply-order :store-id "store-1"
                                  :patch {:item "imported handbags" :quantity 15 :estimated-cost 300.0
    ...
    (let [actor-direct (op/build db {:advisor (reify advisor/Advisor
      ...
      (println (exec-op actor-direct "t10" {:op :schedule-staffing-operation :store-id "store-1"
                                           :patch {:shift "weekday-fitting-room" :date "2026-07-22"}} coordinator-phase-3)))

    (println "\n== log-sales-record store-1, advisor drifts into quality-dispute-resolution/counterfeit-determination scope -> HARD hold, permanent ==")
