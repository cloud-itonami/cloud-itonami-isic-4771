(ns apparelops.operation
  "OperationActor -- one coordination request = one supervised actor run,
  expressed as a langgraph-clj StateGraph. The advisor
  (ApparelRetailAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the ApparelRetailGovernor (:govern)
  ...
  human store operator/apparel-quality coordinator. The approver resumes
...
(defn build
  ...
  `apparelops.store/Store`).
  opts:
    :advisor      -- an `apparelops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; ApparelRetailAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; ApparelRetailGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
