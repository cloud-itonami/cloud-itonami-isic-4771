(ns apparelops.phase
  "Phase 0->3 staged rollout for the ISIC-4771 'Retail sale of clothing,
  footwear and leather articles in specialized stores' operations-
  coordination actor.

    Phase 0  read-only              -- no writes, still governor-gated.
    Phase 1  assisted-logging       -- sales-record logging allowed,
                                       every write needs human approval.
    Phase 2  assisted-coordination  -- adds floor-staff/fitting-room
  ...
  rollout milestone still to come. Flagging a defective-garment/mis-
  sized/mislabeled/suspected-counterfeit concern always needs a human to
  actually look at it. `apparelops.governor`'s own `always-escalate-ops`
  enforces the same invariant independently -- two layers, not one,
  agree on this."
  (:require [apparelops.governor :as governor]))
...
(defn verdict->disposition
  "Map an ApparelRetailGovernor verdict to a base disposition before the
  phase gate."
  [verdict]
